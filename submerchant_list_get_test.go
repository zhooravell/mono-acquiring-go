...
func TestGetSubMerchantList(t *testing.T) {
	mux := http.NewServeMux()
	mux.HandleFunc("/api/merchant/submerchant/list", func(w http.ResponseWriter, req *http.Request) {
  ...
  "list": [
	...
	res, err := client.GetSubMerchantList(context.Background())

	assert.NoError(t, err)
	assert.NotNil(t, res)
	assert.Len(t, res.List, 1)

	assert.Equal(t, "0a8637b3bccb42aa93fdeb791b8b58e9", res.List[0].Code)
	assert.Equal(t, "4242424242", res.List[0].Edrpou)
	assert.Equal(t, "UA213996220000026007233566001", res.List[0].Iban)
	assert.Equal(t, "ТОВ Сергій Корольов", res.List[0].Owner)

}

func TestGetSubMerchantList_NetworkError(t *testing.T) {
